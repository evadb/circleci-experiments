...
const resolvers = {
  ...
  Query: {
    evaluationsFor: async (root, { account, postalCode }, { client }) => {
      let cursor = await client.find({
        $query: { HOUSE_ID: account, CLIENTPCODE: postalCode },
  ...
  Evaluation: {
    ...
    mainwallins: root => root.MAINWALLINS,
    storeys: root => root.STOREYS,
    totalOccupants: root => root.TOTALOCCUPANTS,
    planshape: root => root.PLANSHAPE,
    tbsmnt: root => root.TBSMNT,
    tmain: root => root.TMAIN,
    hsevol: root => root.HSEVOL,
    airLeakageAt50Pascals: root => root.AIR50P,
    leakar: root => root.LEAKAR,
    cenventsystype: root => root.CENVENTSYSTYPE,
    registration: root => root.REGISTRATION,
    programname: root => root.PROGRAMNAME,
    eghfconelec: root => root.EGHFCONELEC,
    eghfconngas: root => root.EGHFCONNGAS,
    eghfconoil: root => root.EGHFCONOIL,
    eghfconprop: root => root.EGHFCONPROP,
    eghfcontotal: root => root.EGHFCONTOTAL,
    eghspaceenergy: root => root.EGHSPACEENERGY,
